...
class CorrectionRecord(Base):
    ...
    validation_result = Column(Boolean, nullable=True)  # True if correct, False if wrong
...
class LearnedRule(Base):
    ...
    # Rule definition
    pattern_template = Column(JSON)  # Conditions for matching
    recommended_action = Column(String)
    base_confidence = Column(Float)

    # Learning metadata
    support_count = Column(Integer)  # Number of corrections that created this
    created_at = Column(Float)

    # Validation tracking (used for dynamic confidence)
...
class ModelVersion(Base):
    ...
    # Model location (could be S3 path, MLflow URI, etc.)
    ...
    # Deployment status
    status = Column(String)  # 'training', 'staging', 'production', 'archived'
