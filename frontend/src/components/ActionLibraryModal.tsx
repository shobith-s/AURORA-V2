            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6 border-b border-slate-100 bg-white z-10">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p className="text-slate-500 text-sm mt-1">Browse and select the perfect preprocessing transformation</p>
                        </div>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-lg"
                            autoFocus
                        />
                    </div>
                </div>
                            ...
                            {categories.map(category => (
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        {category}
                                        {category === 'All' && <span className="text-xs bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded-full">{ACTIONS.length}</span>}
                                    </div>
                                </button>
                            ))}
                        </div>
                        ...
                        {filteredActions.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {filteredActions.map(action => (
                                    ...
                                    >
                                        <div className={`p-3 rounded-lg ${currentAction === action.id ? 'bg-blue-200 text-blue-700' : 'bg-slate-100 text-slate-600 group-hover:bg-blue-100 group-hover:text-blue-600'
                                            } transition-colors`}>
                                            <action.icon className="w-6 h-6" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <h4 className={`font-bold ${currentAction === action.id ? 'text-blue-800' : 'text-slate-800'}`}>
                                                    {action.label}
                                                </h4>
                                                {action.popular && (
                ...
                <div className="p-4 border-t border-slate-100 bg-white flex justify-between items-center text-sm text-slate-500">
                    <div className="flex items-center gap-4">
                        <span className="flex items-center gap-1"><span className="px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200 text-xs font-mono">↑↓</span> Navigate</span>
                        <span className="flex items-center gap-1"><span className="px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200 text-xs font-mono">Enter</span> Select</span>
                        <span className="flex items-center gap-1"><span className="px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200 text-xs font-mono">Esc</span> Close</span>
                    </div>
                    <div>
